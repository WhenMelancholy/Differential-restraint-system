...
class NumberPlaneTest(Scene):
    def construct(self):
        ...
        self.wait()
        grid.prepare_for_nonlinear_transform()
        grid_transform_title = Text("平移")
        grid_transform_title.to_corner(UP)
        grid_transform_title.scale(1.5)
        self.play(
            Transform(grid_title, grid_transform_title),
            grid.apply_function,
            lambda p: p + np.array([2, 1, 0.5, ]),
            run_time=3,
        )
        self.wait()
        grid.prepare_for_nonlinear_transform()
        grid_transform_title = Text("线性变换")
        grid_transform_title.to_corner(UP)
        grid_transform_title.scale(1.5)
        self.play(
            Transform(grid_title, grid_transform_title),
        ...
        self.wait()
        grid.prepare_for_nonlinear_transform()
        grid_transform_title = Text("非线性变换")
        grid_transform_title.to_corner(UP)
        grid_transform_title.scale(1.5)
        self.play(
            Transform(grid_title, grid_transform_title),
...
class VideoStart(Scene):
    ...
    def construct(self):
        ...
        title = Text(self.title_name)
        title.to_corner((DOWN + ORIGIN))
        self.play(
...
class TrySurroundingRectangle(Scene):
    def construct(self):
        ...
        trans_text_1 = Text("TEXT", tex_to_color_map={"TEXT": BLUE})
        trans_text_2 = Text("TEXT", tex_to_color_map={"TEXT": BLUE})
...
class TryMatrix(Scene):
    def construct(self):
        ...
        trans = Text("1").move_to(matrix[0][9])
        self.play(
...
class TransformPartOfMathTex(Scene):
    def construct(self):
        gauss = MathTex(
            "\\def\\ooint{{\\bigcirc}\\kern-12.5pt{\\int}\\kern-6.5pt{\\int}}"
            "\\ooint_S{E\\cdot dS} = {1\\over{\\epsilon_0}}\\times{{\\int}\\kern-6.5pt{\\int}\\kern-6.5pt{\\int}",
            "_{Sh}", "{\\rho \\cdot dV}}",
        )
        self.play(Write(gauss))
        trans_tex = MathTex("_{V}")
        trans_tex.move_to(gauss[1])
        self.play(
            Transform(gauss[1], trans_tex)
        )
        self.wait()
        text = Text("V", tex_to_color_map={"V": BLUE})
